            ...
            steps {
                script {
                    kubeconfig(credentialsId: 'config', serverUrl: 'https://192.168.49.2:8443') {
                        sh '''
                        argocd login 34.45.23.52:31704 --username admin --password $(kubectl get secret -n argocd argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d) --insecure
