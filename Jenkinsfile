pipeline {
    agent any
    environment {
        DOCKER_HUB_REPO = "shubhkesh/studdybuddy"
    ...
    stages {
        ...
        stage('Update Deployment YAML with New Tag') {
            steps {
                script {
                    sh """
                    sed -i 's|image: shubhkesh/studdybuddy:.*|image: shubhkesh/studdybuddy:${IMAGE_TAG}|' manifest/deployment.yaml
                    """
                }
            }
        }
            ...
            steps {
                script {
                    kubeconfig(credentialsId: 'kubeconfig', serverUrl: 'https://192.168.49.2:8443') {
                        sh '''
                        argocd login 104.197.231.20:31704 --username admin --password $(kubectl get secret -n argocd argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d) --insecure
